...
1. ApplicationContext에 Bean 등록 
2. client가 해다 bean을 사용하려고 요청 
3. Bean Factory에 해당 Bean이 있는지 조회
  	- 빈 목록에 있다면 다음으로 넘어감
  	- 없다면 예외 발생
4. 오브젝트 확인(해당 bean의 scope가 싱글톤인 경우)
  	- 만약 해당 빈이 아직 한번도 사용되지 않아 생성되어 있지 않다면 해당 빈을 생성합니다.
  	- 이미 사용되어진 빈이라면 오브젝트가 만들어져 있어 바로 사용합니다. 
...
- https://sabarada.tistory.com/70?category=803157
